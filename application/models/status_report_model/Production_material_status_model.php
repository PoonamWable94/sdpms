...
class Production_material_status_model extends CI_Model
{   
    public function __construct(){
        parent::__construct();
    }

    var $table = 'tg_production_comp_assembly';         
    var $column_order = array(null,'p4.company_name', 'p.projectNo', 'p3.equipment', 'p.tag_number', 'p1.task','p.quantity', 'p.ind_time', 'p.total_time','p5.name','p.clientApproval','p.prod_release','p.mfg_type');
    var $column_search = array('p4.company_name', 'p.projectNo', 'p3.equipment', 'p.tag_number', 'p1.task','p.quantity', 'p.ind_time', 'p.total_time','p5.name','p.clientApproval','p.prod_release','p.mfg_type'); 
    var $order = array('p.activityID' => 'desc');
 
    private function _get_datatables_query()
    {          
        $this->db->select('p.*, p1.task as activity, p2.task_data as sub_activity');
        ...
        $this->db->from('tg_production_comp_assembly as p');
        $this->db->join('tg_production_task as p1','p1.id = p.activity');        
        $this->db->join('tg_production_task_data as p2','p2.id = p.sub_activity');        
          ...
          if($this->input->post('projects_id'))
        {
            $projects_id = $this->input->post('projects_id');
              $this->db->where('p.projectID',$projects_id);   
    ...
    //project detail for dropdown filter
    ...
    public function updateAssemblyActivityLevelOrder($data, $activityID)
    {        
        $this->db->where('activityID', $activityID);
        $this->db->update('tg_production_window_assembly', $data);        
        return $this->db->affected_rows();
    ...
    public function delete_assembly_by_id($activityID)
    {
        $data = array( 
            'isDeleted' => 0
        );
    ...
    {   
        $this->db->select('project_no, productionProjectStartDate, productionProjectEndDate, productionActualStartDate, productionActualEndDate');
        $this->db->from('tg_project_detail');
    ...
    public function getProductionActivityData($projectID,$projectequipment,$client_id){
        $this->db->select('p.activityID, p.activity, p.projectID, p.projectequipment,
         p1.id as taskID, p1.task, p.row_order,p.level, p.supervisor, p.quantity, p.ind_time, p.total_time,
         p.clientApproval, p.prod_release, p.mfg_type, p.total_time_hidden, p.ind_time_hidden');        
    ...
    public function getProductionAssemblyActivityData($projectID,$projectequipment,$client_id){
        $this->db->select('p.activityID, p.activity, p.projectID, p.projectequipment, p1.id as subassemblyID, p1.assembly, p.row_order,p.level, p.supervisor, p.quantity, p.ind_time, p.total_time, p.clientApproval, p.prod_release, p.mfg_type, p.total_time_hidden, p.ind_time_hidden');        
    ...
    public function get_component_assembly_data($project_id, $project_equipment_id, $activity_id, $component_id){
        $this->db->select('p2.task_data, p1.assemblyID, p1.skill, p1.activity_days, p1.activity_time_hours,
         p1.activity_time_minutes, p1.total_time,p1.total_time_save, p1.startDate, p1.targetDate, p1.actual_start_date,
          p1.actual_end_date,p1.startDelay,p1.endDelay, p1.manpower, p1.resp_persons, p1.quality_qc_date,
           p1.quality_qc_remark, p1.tpi_qc_date, p1.tpi_qc_remark,p1.clientApproval, p1.prod_release,
            p1.mfg_type, p1.is_all_updated, p1.activity, p1.sub_activity, p1.sub_act_order');        
        $this->db->from('tg_production_window as p');
        ...
        $where = array('p.isDeleted'=>'1','p.status'=>'1','p1.projectID'=>$project_id,
        'p1.projectequipment'=>$project_equipment_id,'p1.activityID'=>$activity_id,
         'p1.activity'=>$component_id, 'p1.isDeleted'=>'1','p1.status'=>'1');
        $this->db->where($where);         
    ...
    public function get_production_skill_assembly_employee($activityID,$sub_activity_id)
    {           
        $select =   array(
            'p3.name', 'p1.userId', 'count(p1.skillId) as totalSkills','p1.skill'
        ); 
        $this->db->SELECT($select);
        $this->db->from('tg_production_emp_skills AS p1');
        $this->db->join('tg_production_assembly_skills as p2','p2.skill_master_id = p1.skillId');
        $this->db->join('tg_production_emp as p3','p3.userId = p1.userId');
        $where = array('p2.assembly'=>$activityID, 'p2.activityId'=>$sub_activity_id, 'p1.status'=>'1', 'p1.isDeleted'=>'1', 'p2.status'=>'1', 'p2.isDeleted'=>'0', 'p3.status'=>'1', 'p3.isDeleted'=>'0');
        $this->db->where($where);
        $this->db->group_by('p1.userId');   
        $query = $this->db->get();
        return $query->result();     
    ...
    var $table1 = 'tg_production_window_assembly';         
    var $column_order1 = array(null,'p4.company_name', 'p.projectNo', 'p3.equipment', 'p.tag_number', 'p1.assembly','p.quantity', 'p.ind_time', 'p.total_time','p5.name','p.clientApproval','p.prod_release','p.mfg_type');
    var $column_search1 = array('p4.company_name', 'p.projectNo', 'p3.equipment', 'p.tag_number', 'p1.assembly','p.quantity', 'p.ind_time', 'p.total_time','p5.name','p.clientApproval','p.prod_release','p.mfg_type'); 
    var $order1 = array('p.activityID' => 'desc');
 
    private function _get_datatables_query1($projectID)
    {          
    ...
    public function get_datatables1($projectID)
    {
        $this->_get_datatables_query1($projectID);
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    ...
    public function count_filtered1($projectID)
    {
        $this->_get_datatables_query1($projectID);
        $query = $this->db->get();
        return $query->num_rows();
    ...
    public function count_all1($projectID)
    {
        $this->db->from('tg_production_window_assembly');
        $this->db->where('isDeleted',1);
        return $this->db->count_all_results();
    ...
    public function get_assembly_data($project_id, $project_equipment_id, $activity_id, $component_id){
    ...
    public function updateAssemblySubActivityList($data, $assemblyID)
    {        
        $this->db->where('assemblyID', $assemblyID);
        $this->db->update('tg_production_sub_assembly_list', $data);        
        return $this->db->affected_rows();
    ...
    public function update_assembly_client_approval($activityID,$clientApproval)
    {
        $data['clientApproval'] = $clientApproval;
        $this->db->where('activityID', $activityID);
        $this->db->update('tg_production_window_assembly', $data);
    ...
    public function update_prod_assembly_release($activityID,$prod_release)
    {
        $data['prod_release'] = $prod_release;
        $this->db->where('activityID', $activityID);
        $this->db->update('tg_production_window_assembly', $data);
    ...
    public function update_assembly_status($activityID,$status)
    {
        $data['status'] = $status;
        $this->db->where('activityID', $activityID);
        $this->db->update('tg_production_window_assembly', $data);
    ...
    public function delete_assembly_activity_by_id($activityID)
    {
        $data = array( 
            'isDeleted' => 0
        );
    ...
    public function delete_sub_assembly_activity_by_id($activityID)
    {
        $data = array( 
            'isDeleted' => 0
        );
    ...
    public function is_UpdatedActivity_assemblyID($assemblyID){
        
        $this->db->from('tg_production_comp_assembly');

        $where = array('assemblyID'=>$assemblyID);
        $this->db->where($where);         
    ...
    public function is_UpdatedActivity_Sub_assemblyID($assemblyID){
    ...
    public function get_sub_activity(){
        $this->db->select('	p1.id as task_data_id, p1.task, p1.task_data, p1.activity_order,
                            p2.task, p2.id as task_id
        ');

        $this->db->from('tg_production_task_data as p1');
        $this->db->join('tg_production_task as p2', 'p2.id = p1.task');
        $where = array('p1.isDeleted'=>'0','p1.status'=>'1');
        $this->db->where($where); 
    ...
    public function get_assembly_activity(){
        $this->db->select('p1.id as task_data_id, p1.assembly_id, p1.assembly_activity, 
                            p2.assembly, p2.id as task_id
        ');

        $this->db->from('tg_production_sub_assembly as p1');
        $this->db->join('tg_production_assembly as p2', 'p2.id = p1.assembly_id');
        $where = array('p1.isDeleted'=>'1','p1.status'=>'1');
        $this->db->where($where); 
        $this->db->order_by('p1.id','DESC');         
        $query = $this->db->get();
        return $query->result();
    ...
    public function get_project_activity($projectID){

        $this->db->select('
            p.projectID, p.activityID, p.projectNo, p.activity, p.sub_activity , p.is_all_updated, p.sub_activity as activity_data_id,
        ');
        $this->db->from('tg_production_comp_assembly as p');
        $this->db->where('p.isDeleted',1);
    ...
    public function get_project_activity_count($projectID, $activity_id ,$filter_variable){
        ...
        $this->db->where('p.isDeleted',1);
        $this->db->where('p.is_all_updated', $filter_variable);
        $this->db->where('p.activity', $activity_id);
    ...
    public function get_project_assembly_activity($projectID){

        $this->db->select('
            p.projectID, p.activityID, p.projectNo, p.activity, p.sub_activity , p.is_all_updated, p.sub_activity as activity_data_id,
        ');
        $this->db->from('tg_production_sub_assembly_list as p');
        $this->db->where('p.isDeleted',1);
    ...
    public function get_project_assembly_activity_count($projectID, $activity_id ,$filter_variable){
