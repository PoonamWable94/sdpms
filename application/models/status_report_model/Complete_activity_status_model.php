...
class Complete_activity_status_model extends CI_Model
{   
    ...
    {          
        $this->db->select('
            p.activityID, p.projectNo, p.activity, p.skill1,p.person1, p.startDate, p.targetDate, p.delayDays, p.status , p.projectequipment,
            p.tag_number, p.clientApproval, p.is_all_updated, p.activity as activity_data_id,
           
            
            
        ');
        $this->db->from('tg_design_window as p');
        // $this->db->join('tg_activity_data as p1','p1.id = p.activity', 'left');
    ...
    public function get_design_activity_status1($projectID){

        $this->db->select('
            p.projectID, p.activityID, p.projectNo, p.activity, p.skill1,p.person1, p.startDate, p.targetDate, p.delayDays, p.status , p.projectequipment,
            p.tag_number, p.clientApproval, p.is_all_updated, p.activity as activity_data_id,
    ...
    public function get_purchase_component_activity_status($projectID){

        $this->db->select('
            p.projectID, p.projectNo, p.activity, p.is_all_updated,
        ');
    ...
    public function get_purchase_assembly_activity_status($projectID){

        $this->db->select('
            p.projectID, p.activityID, p.projectNo, p.is_all_updated,
        ');
    ...
    public function get_purchase_activity_status($projectID){

        $this->db->select('
            p.projectID, p.projectNo, p.material_reqd_prod
        ');

        $this->db->from('tg_purchase_window as p');
    ...
    {          
        ...
        $this->db->join('tg_equipment as p3','p3.id = p.projectequipment');
        $this->db->join('tg_client as p4','p4.id = p.client');
        $this->db->join('tg_project_detail as p5','p5.id = p.projectID');
    ...
    public function get_activity_by_projectID($projectID)
    {          
        $this->db->select('p4.company_name, p.activityID, p.projectNo, p.activity, p.skill1,p.person1, p.startDate, p.targetDate, p.delayDays, p.status , p1.activity_data AS activity, p2.skills,p.projectequipment,p3.equipment,p.tag_number,p.clientApproval,
                            p.is_all_updated,
                            p5.po_date_time, p5.project_no   
