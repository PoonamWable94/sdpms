...
class Fg_item_master extends BaseController
{
    public function __construct()
    {
        parent::__construct();
         $this->load->model('Fg_item_master_model');
        $this->isLoggedIn();   
    }
    ...
    public function index()
    {
        ...
        $data['main_group'] = $this->Fg_item_master_model->get_main_group();
        $data['sub_group'] = $this->Fg_item_master_model->get_sub_group();
        //  print_r($data['main_group']); die;        
        $this->loadViews("master/inventory_master/fg_item_master",$data, $this->global, NULL , NULL);
    }

    public function get_subgroup_by_maingroup()
    {
        $main_group_id = $this->input->post('main_group_id');
        $subgroup = $this->Fg_item_master_model->get_subgroup_by_maingroup($main_group_id);
        echo json_encode($subgroup);
    }

    public function ajax_list()
    {
        $list = $this->Fg_item_master_model->get_datatables();
        // print_r($list); die;
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Fg_item_master_model->count_all(),
                        "recordsFiltered" => $this->Fg_item_master_model->count_filtered(),
                        "data" => $data,
                );
    ...
    {
        $data = $this->Fg_item_master_model->get_by_id($id);
    ...
    public function ajax_add()
    {
        ...
        $insert = $this->Fg_item_master_model->save($data);
    ...
    public function ajax_update()
    {
        ...
        $this->Fg_item_master_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }
 
    public function ajax_delete($id)
    {
        $this->Fg_item_master_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    }
	...
	{
        $this->Fg_item_master_model->update_status($this->input->post('id'), $this->input->post('status'));
	}
 }
