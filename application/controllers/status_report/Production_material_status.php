...
class Production_material_status extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('status_report_model/Production_material_status_model');
    ...
    {   
        $data['project_list'] = $this->Production_material_status_model->getProjectDetails();
        $data['sub_activity_list'] = $this->Production_material_status_model->get_sub_activity();

        $data['assembly_activity_list'] = $this->Production_material_status_model->get_assembly_activity();

        // echo "<pre>";
        // print_r($data['sub_activity_list']);
        $this->loadViews("status_report_view/production_material_status_listing", $this->global,$data);
    }   
    
    public function project_activity($projectID) {
        $list = $this->Production_material_status_model->get_project_activity($projectID);     
        $activity_list = $this->Production_material_status_model->get_sub_activity(); 
        ...
        foreach ($activity_list as $obj) {
            ...
            foreach ($list as $li) {
                ...
                if ($obj->task_data_id == $li->activity) {
                    $obj->updated_count = count($this->Production_material_status_model->get_project_activity_count($projectID, $obj->task_data_id ,1));
                    $obj->not_updated_count = count($this->Production_material_status_model->get_project_activity_count($projectID, $obj->task_data_id ,0));
    ...
    {    
        // $list = $this->Production_material_status_model->get_datatables();
        ...
        $output = array(
                    "draw" => $_POST['draw'],
                    "recordsTotal" => $this->Production_material_status_model->count_all(),
                    "recordsFiltered" => $this->Production_material_status_model->count_filtered(),
                    "data" => $data, 
                );        
    ...
    // Production Assembly Grid
    public function project_assembly_activity($projectID) {
        $list = $this->Production_material_status_model->get_project_assembly_activity($projectID);     
        $activity_list = $this->Production_material_status_model->get_assembly_activity(); 
        ...
        foreach ($activity_list as $obj) {
            ...
            foreach ($list as $li) {
                ...
                if ($obj->task_data_id == $li->activity) {
                    $obj->updated_count = count($this->Production_material_status_model->get_project_assembly_activity_count($projectID, $obj->task_data_id ,1));
                    $obj->not_updated_count = count($this->Production_material_status_model->get_project_assembly_activity_count($projectID, $obj->task_data_id ,0));
                    break;  // No need to continue once found
                }
            }
        }
    ...
    {    
        ...
        foreach ($projects as $pr) {
            ...
            $activity_list = $this->project_assembly_activity($pr->id);
...
//                 $logData = array(
//                     'mul_tbl_id' => $last_component_id,
//                     'mul_tbl_coordinator' => 'tg_production_window_assembly',                   
//                     'activity_type' => 4,
//                     'from_dept_id' => $this->session->userdata('dept'),
//                     'roleId' => $this->session->userdata('role'),
//                     'userId' => $this->session->userdata('userId'),
