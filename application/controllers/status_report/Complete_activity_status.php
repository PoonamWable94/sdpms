...
class Complete_activity_status extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('status_report_model/Design_material_status_model');
        $this->load->model('status_report_model/Complete_activity_status_model');
        $this->isLoggedIn();   
    ...
    {                   
        ...
        $this->loadViews("status_report_view/complete_activity_status_listing", $this->global, $data);
    }  
    ...
    public function project_activity($projectID) {
        ...
        $design_list = $this->Complete_activity_status_model->get_design_activity_status1($projectID); 

        $design_count_0 = 0;
        $design_count_1 = 0;

        foreach ($design_list as $design_item) {
            if ($design_item->is_all_updated == 0) {
                $design_count_0++;
            } elseif ($design_item->is_all_updated == 1) {
                $design_count_1++;
            }
        }
...
// ------------------------------------------------------------------------------------------------
        ...
        $list_comp_sub_activity = $this->Complete_activity_status_model->get_purchase_component_activity_status($projectID);
                
        $production_comp_sub_count_0 = 0;
        $production_comp_sub_count_1 = 0;

        
        foreach ($list_comp_sub_activity as $prd_comp_item) {
            if ($prd_comp_item->is_all_updated == 0) {
                $production_comp_sub_count_0++;
            } elseif ($prd_comp_item->is_all_updated == 1) {
                $production_comp_sub_count_1++;
            }
        }
        ...
        $list_assembly_sub_activity = $this->Complete_activity_status_model->get_purchase_assembly_activity_status($projectID);         
        
        $production_assembly_sub_count_0 = 0;
        $production_assembly_sub_count_1 = 0;

        foreach ($list_assembly_sub_activity as $asse_sub_item) {
            if ($asse_sub_item->is_all_updated == 0) {
                $production_assembly_sub_count_0++;
            } elseif ($asse_sub_item->is_all_updated == 1) {
                $production_assembly_sub_count_1++;
            }
        }
...
// ------------------------------------------------------------------------------------------------
        ...
        $purchase_list = $this->Complete_activity_status_model->get_purchase_activity_status(195); 

        $reqd_prod_count_selected = 0;
        $reqd_prod_count_not_selected = 0;

        foreach ($purchase_list as $purchase_item) {
            // Assuming '0000-00-00' denotes a default or unspecified date
            if ($purchase_item->material_reqd_prod != '0000-00-00') {
                $reqd_prod_count_selected++;
            } else {
                $reqd_prod_count_not_selected++;
            }
        }
...
// ------------------------------------------------------------------------------------------------
        ...
        $data['design_percentage'] = $this->calculateCompletionPercentage($design_count_0 + $design_count_1, $design_count_1);
        $data['production_assembly_percentage'] = $this->calculateCompletionPercentage($production_assembly_sub_count_0 + $production_assembly_sub_count_1, $production_assembly_sub_count_1);
        $data['production_comp_sub_percentage'] = $this->calculateCompletionPercentage($production_comp_sub_count_0 + $production_comp_sub_count_1, $production_comp_sub_count_1);
        $data['purchase_percentage'] = $this->calculateCompletionPercentage($reqd_prod_count_not_selected + $reqd_prod_count_selected, $reqd_prod_count_selected);
    ...
    public function list_activity() {
        ...
        foreach ($projects as $pr) {
            ...
            $design_percentage = $activity_list['design_percentage'];
            $production_assembly_percentage = $activity_list['production_assembly_percentage'];
            $production_comp_sub_percentage = $activity_list['production_comp_sub_percentage'];
            $purchase_percentage = $activity_list['purchase_percentage'];
            ...
            $row[] = '
                        <div class="uk-progress uk-progress-warning uk-margin-remove">
                            <div class="uk-progress-bar" style="width: ' . $design_percentage . '%;">' . number_format($design_percentage, 2) . '%</div>
                        </div>
            ...
            $row[] = '
                        <div class="uk-progress uk-progress-warning uk-margin-remove">
                            <div class="uk-progress-bar" style="width: ' . $purchase_percentage . '%;">' . number_format($purchase_percentage, 2) . '%</div>
                        </div>
            ...
            $row[] = '
                        ...
                        <div class="uk-progress uk-progress-warning uk-margin-remove">
                            <div class="uk-progress-bar" style="width: ' . $production_comp_sub_percentage . '%;">' . number_format($production_comp_sub_percentage, 2) . '%</div>
                        </div>
                        <br>
                        <div class="uk-progress uk-progress-warning uk-margin-remove">
                            <div class="uk-progress-bar" style="width: ' . $production_assembly_percentage . '%;">' . number_format($production_assembly_percentage, 2) . '%</div>
                        </div>
    ...
    public function calculateCompletionPercentage($totalActivities, $completedActivities) {
