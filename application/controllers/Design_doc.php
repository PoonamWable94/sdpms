...
class Design_doc extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('design_doc_model');
        $this->isLoggedIn();   
    }
    
    public function index()
    {
        $this->global['pageTitle'] = 'Design Documents';
        // $data['design_activity'] = $this->design_activity_model->get_design_activity();
        //print_r($data); die;
        $this->loadViews("master/design_master/design_doc", $this->global, NULL);
    }

    public function ajax_list()
    {
        $list = $this->design_doc_model->get_datatables();
       // print_r($list); die();
        ...
        foreach ($list as $doc)
        {
            ...
            $row[] = '
                ...
                <a href="'.site_url('design_doc/download/' . urlencode($doc->document)).'" data-uk-tooltip title="Download">
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->design_doc_model->count_all(),
                        "recordsFiltered" => $this->design_doc_model->count_filtered(),
                        "data" => $data,
                );
    ...
    public function download($filename) {
        ...
        $file_path = 'uploads/design_doc/' . $filename;
...
public function download_multiple() {
    ...
    $documents = $this->design_doc_model->get_documents_by_ids($ids);
    ...
    foreach ($documents as $doc) {
        $file_path = './uploads/design_doc/' . $doc->document;
        if (file_exists($file_path)) {
    ...
    {
        $data = $this->design_doc_model->get_by_id($id);
       // print_r($data); die();
    ...
    public function ajax_add() {
        ...
        $upload_path = 'uploads/design_doc/';
        ...
        $insert = $this->design_doc_model->save($data);
    ...
    public function ajax_update()
    {
        ...
        $this->design_doc_model->update(array('id' => $this->input->post('id')), $data);
        echo json_encode(array("status" => TRUE));
    }
 
    public function ajax_delete($id)
    {
        $this->design_doc_model->delete_by_id($id);
        echo json_encode(array("status" => TRUE));
    }
	...
	{
        $this->design_doc_model->update_status($this->input->post('id'), $this->input->post('status'));
	}
}
