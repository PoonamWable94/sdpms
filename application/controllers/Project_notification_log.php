...
class Project_notification_log extends BaseController
{
    public function __construct()
    {
        parent::__construct();
        $this->load->model('Project_notification_log_model');
        $this->isLoggedIn();
    ...
    {
        $this->global['pageTitle'] = 'Add New Project';        
        $this->loadViews("project_notification/list_project", $this->global);
    }   

    public function ajax_list()
    {
        $list = $this->Project_notification_log_model->get_datatables();
    //    echo'<pre>'; print_r($list); die;
        ...
        {
            $edit = base_url('project_notification/edit_project/').$lists->id;
        ...
        $output = array(
                    "draw" => $_POST['draw'],
                    "recordsTotal" => $this->Project_notification_log_model->count_all(),
                    "recordsFiltered" => $this->Project_notification_log_model->count_filtered(),
                    "data" => $data,
                );        
    ...
    {    
        ...
        {              
            ...
            $this->loadViews("project_notification/add_project", $this->global, NULL);
        }
        else{ 
            ...
            $lastInsertId = $this->Project_notification_log_model->save($data);
                                                          
            redirect(base_url('Project_notification_log'));            
        }
    }

    public function edit_data($id)
    {    
        $data['pdata'] = $this->Project_notification_log_model->get_by_id($id);          
        $this->loadViews('project_notification/edit_project',$this->global, $data,NULL);                      
    }

    public function view_data($id)
    {              
        $data = $this->Project_notification_log_model->get_project_view($id);                           
    ...
    public function update_project()
    {
        ...
        if($this->form_validation->run() == false)
        {                  
            $data['pdata'] = $this->Project_notification_log_model->get_by_id($pid);                               
            $this->loadViews("project_notification/edit_project", $this->global, $data , NULL);
        }
        else{                  
            ...
            $this->Project_notification_log_model->update(array('id' => $pid), $data);                            
            redirect(base_url('Project_notification_log'));
        }
    }
    ...
    {
        $this->Project_notification_log_model->delete_by_id($id);
	...
	{
        $this->Project_notification_log_model->update_status($this->input->post('id'), $this->input->post('status'));
	}   
