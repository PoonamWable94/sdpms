    ...
    <div id="page_content_inner">        
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                <div class="md-card-toolbar-heading-text"> Project notification List                    
                    <?php if($this->session->userdata('role') != 4) { ?>
                        <a href="<?php echo base_url('project_notification/add_project');?>" class="spacing-tab" data-uk-tooltip="{pos:'bottom'}" title=""><i class="md-icon material-icons md-color-white">&#xE146;</i></a> 
                    <?php } ?>        
                </div>
            </div>
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Project notification Details</h3>
                </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {            
        table = $('#ts_issues4').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('Project_notification_log/ajax_list')?>",
                "type": "POST"
            },
    ...
    {    
        //   console.log(id);
        $.ajax({
            url : "<?php echo site_url('Project_notification_log/view_data')?>/" + id,
            type: "GET",
            dataType: "JSON",
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg + "this record"))
        {
            ...
            var myurl="<?php echo base_url()."Project_notification_log/update_status"?>";
            // console.log(id);
            $.ajax({
                type:"POST",
                url:myurl,
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('Project_notification_log/ajax_add')?>";
        } else {
            url = "<?php echo site_url('Project_notification_log/ajax_update')?>";
        }
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure you want to delete this call?', function(){ 
            $.ajax({
                url : "<?php echo site_url('Project_notification_log/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('Project_notification_log/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
