        ...
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                ...
                <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a>
                    ...
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Raw Material"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>  
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Raw Material Details</h3>
                </div>
                    ...
                    <div class="uk-grid">
                        ...
                        <div class="uk-grid">
                            ...
                            <div class="parsley-row md-input-wrapper">
                                <label for="fg_name">Semi Finish Good<span class="req">*</span></label>
                                    <select name="sfg_name" id="sfg_name" rows="3" cols="5" class="main_group md-input label-fixed" required >
                                        <option value="">Select</option>
                                    <?php                                    
                                            foreach ($semi_finish_good as $sfg):
                                            
                                                echo '<option value="'.$sfg->id.'">'.$sfg->sfg_name.'</option>';
                                            endforeach; 
                                    ?>
                                    </select>
                                ...
                                <div class="parsley-row md-input-wrapper">
                                    <label for="name">Raw Material<span class="req">*</span></label>
                                    <input type="text" name="rm_name" id="rm_name" required class="md-input label-fixed" />
                                    <span id="errortag" class="uk-text-danger"></span>
                                </div>
                            </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('Raw_Material/ajax_list')?>",
                "type": "POST"
            },
            "pageLength": 100,
            "columnDefs": [
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."Raw_Material/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('Raw_Material/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id); 
                $('[name="sfg_name"]').val(data.sfg_id);
                $('[name="rm_name"]').val(data.rm_name);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('Raw_Material/ajax_add')?>";
        } else {
            url = "<?php echo site_url('Raw_Material/ajax_update')?>";
        }
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('Raw_Material/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('Raw_Material/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
