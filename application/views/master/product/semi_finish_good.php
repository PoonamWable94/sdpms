        ...
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                ...
                <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a>
                     List of All Semi Finish Good
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Semi_Finish_Good"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>   
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Semi Finish Good Details</h3>
                </div>
                ...
                <form action="#" id="form" class="uk-margin-top">
                    ...
                    <div class="uk-grid">
                      <div class="uk-width-1-1">
                                <div class="parsley-row md-input-wrapper">
                                   <label for="fg_name">Finish Good<span class="req">*</span></label>
                                    <select name="fg_name" id="fg_name" rows="3" cols="5" class="main_group md-input label-fixed" required >
                                        <option value="">Select</option>
                                        <?php                                    
                                            foreach ($finish_good as $fg):
                                                echo '<option value="'.$fg->id.'">'.$fg->fg_name.'</option>';
                                            endforeach; 
                                        ?>
                                    </select>
                    ...
                    <div class="uk-grid">
                            <div class="uk-width-1-1">
                                <div class="parsley-row md-input-wrapper">
                                    <label for="name">Semi Finish Good<span class="req">*</span></label>
                                    <input type="text" name="sfg_name" id="sfg_name" required class="md-input label-fixed" />
                                    <span id="errortag" class="uk-text-danger"></span>
                                </div>
                            </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('Semi_Finish_Good/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."Semi_Finish_Good/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('Semi_Finish_Good/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id);                
                $('[name="fg_name"]').val(data.fg_id);
                $('[name="sfg_name"]').val(data.sfg_name);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('Semi_Finish_Good/ajax_add')?>";
        } else {
            url = "<?php echo site_url('Semi_Finish_Good/ajax_update')?>";
        }
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('Semi_Finish_Good/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('Semi_Finish_Good/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
