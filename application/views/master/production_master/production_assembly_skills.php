        ...
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                ...
                <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a>  
                    List of All Production Assembly Skill 
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Skills"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>
                ...
                <table id="dt_mytable" class="uk-table uk-table-striped uk-table-hover uk-table-condensed">
                    <thead>
                        <tr>
                            ...
                            <th>Sr.No</th>
                            <th>Assembly Activity</th>
                            <th>Sub Assembly Activity</th>
                            <th>Skill</th>
                            <th>Status</th>
            ...
            <div class="uk-modal-dialog">
                ...
                <form action="#" id="add_prod_act_skill" class="uk-margin-top">
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <select name="assembly_id" id="assembly_id" rows="3" cols="5" class="main_group md-input label-fixed" required>
                                   ...
                                   <?php                                    
                                        foreach ($production_assembly_list as $assembly):                                           
                                            echo '<option value="'.$assembly->id.'">'.$assembly->assembly.'</option>';
                                        endforeach; 
                                    ?>
                                </select>
                    ...
                    <div class="uk-grid">                        
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                            <label for="sub_assembly_id">Sub Assembly Activity<span class="req">*</span></label>
                                <select name="sub_assembly_id" id="sub_assembly_id" rows="3" cols="5" class="main_group md-input label-fixed" required >
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('production_assembly_skills/ajax_list')?>",
                "type": "POST"
            },
        ...
        $('#assembly_id').change(function(){  
            var assembly_id = $(this).val();
            // console.log(projectID);
            if(assembly_id > 0){                   
                $.ajax({
                    url : base_url+'production_assembly_skills/getTaskActivity',
                    type: "POST",
                    data: {'assembly_id':assembly_id},
                    dataType: "json",                    
                    success: function(data){ 
                        // console.log(data);                                                 
                        $('#sub_assembly_id').html(data);
                    },
                });    
            } else{
                $('#sub_assembly_id').html();
            }
        });        
    });
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."production_assembly_skills/update_status"?>";
    ...
    function save()
    {
        ...
        var url;
        if(save_method == 'add') {
            url = "<?php echo site_url('production_assembly_skills/ajax_add')?>";
        } else {
            url = "<?php echo site_url('production_assembly_skills/ajax_update')?>";
        }
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('production_assembly_skills/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                console.log(data);
                $('[name="id"]').val(data.id);
                $('#assembly_id').val(data.assembly);                
                $('#skills').val(data.skill_master_id);
               // $('#assembly_id').attr("disabled", "disabled");

                if(data.assembly > 0){                   
                    $.ajax({
                        url : base_url+'production_assembly_skills/getTaskActivity_edit',
                        type: "POST",
                        data: {'assembly_id':data.assembly, 'activityId':data.activityId},
                        dataType: "json",                    
                        success: function(data){ 
                            // console.log(data);                                                 
                            $('#sub_assembly_id').html(data);
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('production_assembly_skills/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('production_assembly_skills/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
