        ...
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                ...
                <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a>  
                    ...
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Skill"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Production Skill Details</h3>
                </div>
                ...
                <form action="#" id="form" class="uk-margin-top">
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <input type="text" id="skill" name="skill" required class="md-input label-fixed" />
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('production_skill_master/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."production_skill_master/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('production_skill_master/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('production_skill_master/ajax_add')?>";
        } else {
            url = "<?php echo site_url('production_skill_master/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="skill"]').val(data.skills);
                // Disable text box...
                // $('[name="skill"]').attr("disabled", "disabled");
                $("#btnSave").hide();
    ...
    {
        UIkit.modal.confirm('Are you sure to delete thi Skill?', function(){ 
            $.ajax({
                url : "<?php echo site_url('production_skill_master/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
    ...
    function bulk_delete()
    {
        ...
        if(list_id.length > 0)
        {
            UIkit.modal.confirm('Are you sure delete this '+list_id.length+' skill?', function(){ 
            // if(confirm('Are you sure delete this '+list_id.length+' data?'))
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('production_skill_master/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
