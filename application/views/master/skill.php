        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Skill Details</h3>
                </div>
                    ...
                    <div class="uk-grid">
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <input type="text" name="skill" id="skill" required class="md-input label-fixed" />
                                <span id="errortag" class="uk-text-danger"></span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('Skill/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."Skill/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('Skill/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            ...
            url = "<?php echo site_url('Skill/ajax_update')?>";
        }
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('Skill/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
    ...
    if(list_id.length > 0) {
        UIkit.modal.confirm('Are you sure delete this '+list_id.length+' data?', function(){ 
            $.ajax({
                type: "POST",
                data: {id: list_id},
                url: "<?php echo site_url('Skill/ajax_bulk_delete')?>",
                dataType: "JSON",
                success: function(data) {
                    if(data.status) {
                        reload_table();
