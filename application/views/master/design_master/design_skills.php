        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Add Design Activity Skills</h3>
                </div>
                ...
                <form action="#" id="form" class="uk-margin-top">
                    ...
                    <div class="uk-grid">                         
                        <div class="uk-width-1-1">
                            ...
                            <label for="skill">Activity Data<span class="req">*</span></label>
                                <select id="activity" name="activity" data-md-selectize-inline data-md-selectize-bottom data-uk-tooltip="{pos:'top'}" title="Select Activity">                                        
                                    <?php                                                                                    
                                        foreach ($design_activity as $activity) {?>                                    
                                            <option value="<?php echo $activity->id; ?>"<?=$this->input->post('activity') == $activity->id ? ' selected="selected"' : '';?>><?php echo $activity->activity_data; ?></option>
                                        <?php } 
                                    ?>                                       
                                </select>                                
                    ...
                    <div class="uk-grid">                         
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-filled">
                            <label for="skill">Skill Master<span class="req">*</span></label>
                            <select name="skills[]" id="skills" data-md-selectize-inline data-md-selectize-bottom multiple>                                        
                                <?php                                                                                    
                                    foreach ($skillist as $skill){ ?>
                                        <option value="<?php echo $skill->id; ?>"<?=$this->input->post('skills') == $skill->id ? ' selected="selected"' : '';?>><?php echo $skill->skills; ?></option>
                                <?php } ?>                                      
                            </select>                                
                            <span id="errortag" class="uk-text-danger"></span>
                            </div>
...
<script type="text/javascript">   
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('design_skills/ajax_list')?>",
                "type": "POST"
            },
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('design_skills/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {  
                ...
                $('#skills')[0].selectize.setValue(data.skill_master_id);

                // $('#skills')[0].selectize.setValue(data.skills);
                // $('[name="skills"]').val(data.skills);
                // $('[name="activity"]')[0].selectize.disable();
                UIkit.modal($('.uk-modal'), {bgclose: false}).show();
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('design_skills/ajax_add')?>";
        } else {
            url = "<?php echo site_url('design_skills/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="id"]').val(data.id);
                $('[name="activity"]')[0].selectize.setValue(data.design_activity);
                $('[name="skills"]')[0].selectize.setValue(data.skills);
                // $('[name="skills"]').val(data.skills);
                // Disable text box...
                // $('[name="skills"]').attr("disabled", "disabled");
                $('[name="activity"]')[0].selectize.disable();
                $('[name="skills"]')[0].selectize.disable();
                $("#btnSave").hide();
                UIkit.modal($('.uk-modal-edit')).show();
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('design_skills/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('design_skills/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
