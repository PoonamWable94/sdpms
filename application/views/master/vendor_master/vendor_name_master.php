<div id="page_content">    
    <div id="page_content_inner">                
        ...
        <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
            <h1 class="md-card-toolbar-heading-text"> All Vendors 
                <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Vendor"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>
                    ...
                    <div class="uk-grid">                    
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <label for="name">Vendor Name<span class="req">*</span></label>
                                <input type="text" id="name" name="name" required class="md-input label-fixed" />
                    ...
                    <div class="uk-grid">                    
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <select name="location" id="location dept" data-md-selectize-inline data-md-selectize-bottom data-uk-tooltip="{pos:'top'}"  title="Select location">
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('vendor_name/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."vendor_name/update_status"?>";
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('vendor_name/ajax_add')?>";
        } else {
            url = "<?php echo site_url('vendor_name/ajax_update')?>";
        }
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('vendor_name/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                ...
                $('[name="location"]')[0].selectize.setValue(data.location);                
                UIkit.modal($('.uk-modal'), {bgclose: false}).show();
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('vendor_name/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('vendor_name/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
