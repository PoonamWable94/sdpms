    ...
    <div id="page_content_inner">        
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                <h1 class="md-card-toolbar-heading-text"> Condition performace List
                ...
                <table id="dt_mytable" class="uk-table uk-table-striped uk-table-hover uk-table-condensed">
                    <thead>
                        <tr>
                            ...
                            <th>#</th>
                            <th>Condition To Perform Test</th>
                            <th>Status</th>
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Condition To Perform Test</h3>
                </div>
                    ...
                    <div class="uk-grid">
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <label for="quality_condition_to_perform">Condition to perform test<span class="req">*</span></label>
                                <input type="text" name="quality_condition_to_perform" required class="md-input label-fixed lettersOnly" />
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('quality_condition_to_perform/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."quality_condition_to_perform/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('quality_condition_to_perform/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id);
                $('[name="quality_condition_to_perform"]').val(data.quality_condition_to_perform);
                UIkit.modal($('.uk-modal'), {bgclose: false}).show();
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('quality_condition_to_perform/ajax_add')?>";
        } else {
            url = "<?php echo site_url('quality_condition_to_perform/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="quality_condition_to_perform"]').val(data.quality_condition_to_perform);
                // Disable text box...
                $('[name="quality_condition_to_perform"]').attr("disabled", "disabled");
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('quality_condition_to_perform/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('quality_condition_to_perform/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
