    ...
    <div id="page_content_inner">        
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                <h1 class="md-card-toolbar-heading-text"> List Quality Activity Data 
            ...
            <div class="uk-modal-dialog">
                ...
                <form action="#" id="form" class="uk-margin-top">
                    <div class="uk-grid">
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <label for="activity_id">Activity Master<span class="req">*</span></label> 
                                <select name="activity_id" id="activity_id dept" data-md-selectize-inline data-md-selectize-bottom data-uk-tooltip="{pos:'top'}"  title="Select Activity">
                    ...
                    <div class="uk-grid">                        
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <label for="activity">Activity Data<span class="req">*</span></label>                                 
                                <input type="text" id="activity" name="activity" required class="md-input label-fixed" />
                                <span id="errortag" class="uk-text-danger"></span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('quality_activity_data/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."quality_activity_data/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('quality_activity_data/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id);
                $('[name="activity_id"]')[0].selectize.setValue(data.activity_id);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('quality_activity_data/ajax_add')?>";
        } else {
            url = "<?php echo site_url('quality_activity_data/ajax_update')?>";
        }
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('quality_activity_data/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('quality_activity_data/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
