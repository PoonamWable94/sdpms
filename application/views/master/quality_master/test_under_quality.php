    ...
    <div id="page_content_inner">        
        <div class="md-card">
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                <h1 class="md-card-toolbar-heading-text"> Test Under Quality List
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Equipment"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Quality Test</h3>
                </div>
                ...
                <form action="#" id="form" class="uk-margin-top">
                    ...
                    <input type="hidden" value="" name="id"/>
                     <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <select name="test" id="select_demo_5 dept" data-md-selectize-inline data-md-selectize-bottom data-uk-tooltip="{pos:'top'}"  title="Select Test">
                                    <option value="">Select Test</option>
                                   <?php
                                    
                                        foreach ($quality_test as $activity):
                                           
                                            echo '<option value="'.$activity->id.'">'.$activity->test.'</option>';
                                        endforeach; 
                                    ?>
                                </select>
                    ...
                    <div class="uk-grid">
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                <label for="test_under_quality">Test Under Quality<span class="req">*</span></label>
                                 
                                <input type="text" name="test_under_quality" required class="md-input label-fixed" />
                                <span id="errortag" class="uk-text-danger"></span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('test_under_quality/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."test_under_quality/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('test_under_quality/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id);
                $('[name="test_under_quality"]').val(data.test_under_quality);
                $('[name="test"]')[0].selectize.setValue(data.test);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('test_under_quality/ajax_add')?>";
        } else {
            url = "<?php echo site_url('test_under_quality/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="test_under_quality"]').attr("disabled", "disabled");
                $('[name="test"]').attr("disabled", "disabled");
                $("#btnSave").hide();
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('test_under_quality/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('test_under_quality/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
