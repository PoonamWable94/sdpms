        ...
        <div class="md-card">            
            <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
                ...
                <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a> 
                    List of All Purchase Skill
                    <a href="javascript:void(0)" onclick="add_data()" data-uk-tooltip="{pos:'bottom'}" title="Add New Purchase Skill"><i class="md-icon material-icons md-color-white">&#xE146;</i></a>
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Purchase Skills Details</h3>
                </div>
                ...
                <form action="#" id="form" class="uk-margin-top">
                    <div class="uk-grid">
                     ...
                     <div class="uk-width-1-1">
                            ...
                            <label for="activity">Activity<span class="req">*</span></label>
                                <select name="activity" id="select_demo_5 dept" rows="3" cols="5" class="main_group md-input label-fixed" required >
                                    <option value="">Select</option>
                                   <?php
                                    
                                        foreach ($purchase_activity as $skills):
                                           
                                            echo '<option value="'.$skills->id.'">'.$skills->purchase_activity.'</option>';
                                        endforeach; 
                                    ?>
                                </select>
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <input type="text" name="skills" required class="md-input label-fixed lettersOnly" />
                                <span id="errortag" class="uk-text-danger"></span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('purchase_skills/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."purchase_skills/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('purchase_skills/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
                $('[name="id"]').val(data.id);
                $('[name="activity"]').val(data.purchase_activity);
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('purchase_skills/ajax_add')?>";
        } else {
            url = "<?php echo site_url('purchase_skills/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                console.log(data.design_activity);
                $('[name="id"]').val(data.id);
               $('[name="activity"]')[0].selectize.setValue(data.purchase_activity);
                $('[name="skills"]').val(data.skills);
                // Disable text box...
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('purchase_skills/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('purchase_skills/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
