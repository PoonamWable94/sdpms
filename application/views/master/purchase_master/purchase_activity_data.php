    ...
    <div id="page_content_inner">        
        ...
        <div class="md-card-toolbar md-bg-cyan-300" data-toolbar-progress="100">
            ...
            <a href="javascript:void(0);"  onclick="goBack()" title="Back" ><i class="md-icon material-icons md-color-white">keyboard_backspace</i></a> 
                List of All Purchase Activity Data
        ...
        <div class="uk-modal" id="modal_default">
            <div class="uk-modal-dialog">
                ...
                <div class="uk-modal-header">
                    <h3 class="uk-modal-title">Purchase Activity Data Details</h3>
                </div>
                    ...
                    <input type="hidden" value="" name="id"/>
                     <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                             <label for="activity_data">Activity<span class="req">*</span></label>
                                <select name="activity" id="select_demo_5 dept" rows="3" cols="5" class="main_group md-input label-fixed" required >
                                    <option value="">Select Activity</option>
                                   <?php
                                    
                                        foreach ($purchase_activity as $activity):
                                           
                                            echo '<option value="'.$activity->id.'">'.$activity->purchase_activity.'</option>';
                                        endforeach; 
                                    ?>
                                </select>  
                        ...
                        <div class="uk-width-1-1">
                            <div class="parsley-row md-input-wrapper">
                                ...
                                <input type="text" name="activity_data" required class="md-input label-fixed lettersOnly" />
                                <span id="errortag" class="uk-text-danger"></span>
                            </div>
                        </div>
                    </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        ...
        table = $('#dt_mytable').DataTable({ 
            ...
            "ajax": {
                "url": "<?php echo site_url('purchase_activity_data/ajax_list')?>",
                "type": "POST"
            },
    ...
    $(document).on('click','.status_checks',function()
    {
        ...
        if(confirm("Are you sure to "+ msg))
        {
            ...
            var myurl="<?php echo base_url()."purchase_activity_data/update_status"?>";
    ...
    function edit_data(id)
    {
        ...
        $.ajax({
            url : "<?php echo site_url('purchase_activity_data/ajax_edit')?>/" + id,
            type: "GET",
            dataType: "JSON",
            success: function(data)
            {
    ...
    function save()
    {
        ...
        if(save_method == 'add') {
            url = "<?php echo site_url('purchase_activity_data/ajax_add')?>";
        } else {
            url = "<?php echo site_url('purchase_activity_data/ajax_update')?>";
        }
    ...
    function view_data(id)
    {
        ...
        $.ajax({
            ...
            success: function(data)
            {
                ...
                $('[name="activity_data"]').attr("disabled", "disabled");
                $("#btnSave").hide();
    ...
    function delete_data(id)
    {
        UIkit.modal.confirm('Are you sure delete?', function(){ 
            $.ajax({
                url : "<?php echo site_url('purchase_activity_data/ajax_delete')?>/"+id,
                type: "POST",
                dataType: "JSON",
                success: function(data)
                {
        ...
        if(list_id.length > 0)
        {
            ...
            // {
                $.ajax({
                    type: "POST",
                    data: {id:list_id},
                    url: "<?php echo site_url('purchase_activity_data/ajax_bulk_delete')?>",
                    dataType: "JSON",
                    success: function(data)
                    {
                        if(data.status)
